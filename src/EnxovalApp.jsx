...
function makeUnit() { return { photo:null, noPhoto:false, tags:[], note:"", price:"", currency:"BRL", cotacao:"" }; }
...
function unitPriceBRL(unit) {
  const p = parseFloat(unit.price)||0;
  if(!p) return 0;
  if(unit.currency==="USD") {
    const c = parseFloat(unit.cotacao)||0;
    return c > 0 ? p * c : 0; // 0 if no cotacao set yet
  }
  return p;
}
function fmtUSD(v) { return `US$ ${parseFloat(v).toFixed(2).replace(".",",")}` }
...
function ItemCard({ item, priorities, entries, customTags, deletedPresets, onChangeEntry, onCreateTag, onDeleteItem, onUpdateItem, accent }) {
  ...
  const unitSpent = item.sizes.reduce((s,sz)=>
    s+(entries[`${item.id}_${sz.tam}`]?.units||[]).reduce((a,u)=>a+unitPriceBRL(u),0),0);
...
function Finance({ categories, items, entries, priorities, accent,
  ...
  const buckets = FINANCE_BUCKETS.map(bk=>{
    ...
    items.forEach(item=>{
      ...
      const unitPrices = item.sizes.flatMap(sz=>(entries[`${item.id}_${sz.tam}`]?.units||[]).map(u=>unitPriceBRL(u))).filter(v=>v>0);
...
// TAG MANAGER ROW
// ─────────────────────────────────────────────────────────────────────────────
function TagManagerRow({ tag, isPreset, onEdit, onDelete, usageCount, accent }) {
  ...
  return (
    <div style={{
      background:T.bgCard, border:`1px solid ${confirming?T.rose+"66":T.border}`,
      ...
      {/* DELETE CONFIRMATION */}
      {confirming && (
        <div style={{padding:"10px 12px",background:T.rose+"0D"}}>
          <div style={{...mono,fontSize:11,color:T.rose,marginBottom:6,fontWeight:600}}>
            Excluir tag "{tag.label}"?
          </div>
          {usageCount>0 ? (
          ...
          <div style={{display:"flex",gap:6}}>
            <button onClick={()=>{ onDelete(); setConfirming(false); }} style={{
              ...
              {usageCount>0?"Excluir mesmo assim":"Excluir"}
            </button>
            <button onClick={()=>setConfirming(false)} style={{
              padding:"8px 14px",background:T.bgSurf,color:T.inkL,
              border:"none",borderRadius:3,...mono,fontSize:11,cursor:"pointer"
            }}>Cancelar</button>
          </div>
        </div>
      )}
      ...
      {!editing && !confirming && (
        <div style={{display:"flex",alignItems:"center",gap:9,padding:"10px 12px"}}>
          ...
          {isPreset
            ? <span style={{...mono,fontSize:10,color:T.ghost}}>padrão</span>
            : usageCount>0
              ? <span style={{...mono,fontSize:10,color:T.inkLL}}>{usageCount} uso{usageCount!==1?"s":""}</span>
              : null
          }
          <span style={{flex:1}}/>
